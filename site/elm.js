...
var $author$project$Main$RecordTime = function (a) {
	return {$: 'RecordTime', a: a};
};
...
var $author$project$Main$init = F3(
	function (flags, url, key) {
		return A2(
			...
			A2($elm$core$Task$perform, $author$project$Main$RecordTime, $elm$time$Time$now),
			{
				dollarsPerOz: 76.83,
				...
				url: url,
				valid: false,
				validTime: $elm$time$Time$millisToPosix(0)
			});
	});
...
var $author$project$Main$subscriptions = function (model) {
	return A2($elm$time$Time$every, 1000, $author$project$Main$RecordTime);
};
...
var $author$project$Main$update = F2(
	function (msg, model) {
		switch (msg.$) {
			...
			case 'RecordTime':
				var posix = msg.a;
				var passed = $elm$time$Time$posixToMillis(posix) - $elm$time$Time$posixToMillis(model.validTime);
				var now = $elm$time$Time$posixToMillis(posix);
				var minutesPassed = (passed / (1000 * 60)) | 0;
				return $Janiczek$cmd_extra$Cmd$Extra$withNoCmd(
					_Utils_update(
						model,
						{now: posix, valid: minutesPassed < 5}));
			case 'OnUrlChange':
				var url = msg.a;
				return $Janiczek$cmd_extra$Cmd$Extra$withNoCmd(model);
			case 'OnUrlRequest':
